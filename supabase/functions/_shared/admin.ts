...
export async function requireAdmin(request: Request, workspaceId: string) {
  const result = await requireMember(request, workspaceId);
  if (result.membership.role !== 'admin') throw new Error('forbidden');
  return result;
}

export async function requireMember(request: Request, workspaceId: string) {
